	SUBTTL	COMMAND SCANNER MACROS   %7D(303)   23-AUG-85

	IFDEF	%.C,<IFE %.C,<XLIST>>
	IFNDEF	%.C,<XLIST>

CUSTVR==2		;CUSTOMER VERSION  (JMS at TYMSHARE)
DECVER==7		;DEC VERSION
DECMVR==4		;DEC MINOR VERSION
DECEVR==303		;DEC EDIT VERSION




;COPYRIGHT (c) DIGITAL EQUIPMENT CORPORATION 1971,1974,1981,1984,1985,1986. ALL RIGHTS RESERVED.
;
;
...
;106	ADD FS.IFI TO SIGNIFY INDIRECT FILE ILLEGAL IN FILE SPEC
  ;
;(Missing edits) (I think 201-203 came from TOPS-20 land)
;201	Simplify DOSCAN analysis (no change in call)
;TYM0	Remove edit 201, it does not look right.
;202	Add .FLASK, .FLTEL, .FLQUI
;203	Eliminate SCNMAC.REL for UNIVERSAL
;(Missing edits)
  ;
;300			PY	18-Aug-81
...
;CSM/STC changes to SCNMAC
;CSM1	Add support for the /SCAN switch.
;CSM2	Move the .LKWLD and .SCWLD flags into SCNMAC.
;CSM3	Add a flag which forces a switch name to be exact.
;RDH	FT$RSX defines 9 character file names.  This edit extracted from
;	ACCSCM.MAC on the TOOLS tape, and is applicable to the C-LINK
;	program from NIH.
	ND FT$RSX,0	;[RDH] Non-zero for 9-character file names
  ;
;TYMSHARE changes to SCNMAC
;TYM1	Define .FXUSN for 12-character username and .FXHOS for TYMNET
;	host number.  These overlay end of SFD spec.
...
;   17=4th SFD mask, 20=5th SFD, 21=5th SFD mask.  [TYM1]
.FXUSN==17	;USERNAME (2 WORDS) [A.BLAM 15-10-80][Carl 10-Feb-84]
.FXHOS==21	;HOST COMPUTER      [A.BLAM 15-10-80][Carl 10-Feb-84]
;****** end of directory mask field ******
.FXBFR==22	;/BEFORE
.FXSNC==23	;/SINCE
.FXABF==24	;/ABEFORE
.FXASN==25	;/ASINCE
...
.FXEST==30	;/ESTIMATE
.FXVER==31	;/VERSION
IFN FT$RSX,<	;[RDH] 9 character file names for CLINK and NIK
.FXNA1==32	;[RDH] File name part 2
.FXNM1==33	;[RDH] File name mask part 2
.FXNOD==34	;[RDH] Node name
.FXLEN==35	;[RDH] Length of one specification
>  ;End of IFN FT$RSX
IFE FT$RSX,<	;[RDH] Normal TOPS-10 SCAN
...
FX.SCN==3B13	;/SCAN						  [CSM1]
	.FXSFD==1 ;Scan SFDs before UFD (to DELETE *.*[,,*,*,*]). [CSM1]
	.FXUFD==2 ;Scan UFD before starting SFDs (like DIRECT).	  [CSM1]
	.FXUP== 3 ;Set SCAN bit in PATH. block so monitor will	  [CSM1]
		  ; search higher SFDs if file not found.	  [CSM1]
		  ;If set on call to WILD, scan down each SFD	  [CSM1]
		  ; as it is encountered (like BACKUP).		  [CSM1]
;;    ==177B20	;Unused bits
...
;Flags for .LKWLD call						[CSM2]

FW.DAD==1B0	;Do all devices					[CSM2]
FW.UFD==1B1	;Scan UFD before SFD				[CSM2]
FW.XXX==1B2	;Reserved for version 10			[CSM2]
FW.SSN==1B3	;Search on [1,4] after [1,5] for SYS/NEW	[CSM2]
FW.SFD==1B4	;Scan SFD before UFD				[CSM2]
FW.SCN==1B5	;Scan up the path				[CSM2]

;Flags for .SCWLD call						[CSM2]

FW.TWR==1B0	;Use traditional type wildcard replacement	[CSM2]
FW.SDP==1B1	;Supply default path				[CSM2]
...
; SL  NAME,STORAGE,TABLE-NAME,DEFAULTS
...
IFE ..TEMP,<
	...
	>
>>>
;[TYM0]; IF2,<	;Edit [201] modified this slightly
;[TYM0];			;;TABLE OF BYTE POINTERS TO STORE RESULT
;[TYM0]; DEFINE	X($NAME,$PROC,$POINT,$DEFLT,$MAX,$FLAGS),<
;[TYM0]; ..TEMP==0
;[TYM0]; IRP $POINT,<
;[TYM0];     IFE ..TEMP-1,<;;If "*P" then use byte pointer
;[TYM0];	$POINT			;$NAME
;[TYM0];     >
;[TYM0];     IFE ..TEMP-2,<;;If "*F" then use fullword pointer
;[TYM0];	POINT	36,$POINT,35	;$NAME
;[TYM0];     >
;[TYM0];     IFIDN <$POINT><*P>,<..TEMP==1>	;;PARTIAL WORD
;[TYM0];     IFIDN <$POINT><*F>,<..TEMP==2>	;;FULL WORD
;[TYM0]; >;;End IRP
;[TYM0]; IFB <$POINT>,<;;Zero if nothing specified
;[TYM0];	Z			;$NAME
;[TYM0]; >
;[TYM0]; IFNB <$POINT>,<IFE ..TEMP,<				[201]
;[TYM0];	$POINT			;$NAME
;The above does not generate POINT 36,$POINT,35 when it should. [TYM0]
;[TYM0]; > ;End IFE > ;End IFNB					[201]
;[TYM0]; > ;End DEFINE > ;End IF2				[201]

$WIDTH=100
$LENGTH=55
$TITLE PASTSM.PAS, last modified 1/19/84, zw
MODULE pastsm OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- type semantics*)
...
function make_array_type ( arr_defn: parse_node; pack_it: boolean; flex_ok: boolean ): typ;
 ...
 begin
  ...
  with make_array_type^ do begin
    ...
    if node^.next = nil
      then element_type := type_semantics (node, [])    (* component *)
 ...
 begin
  ...
  case typetree^.sym of
    ...
    subr_type: BEGIN
      ...
      IF (pfsym^.next <> NIL) ANDIF (pfsym^.next^.sym = parm_list)
      THEN BEGIN parmlst := pfsym^.next; rettype := parmlst^.next END
      ELSE BEGIN parmlst := NIL; rettype := pfsym^.next END;
      (* FALSE => parms not required *)

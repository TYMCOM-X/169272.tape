$WIDTH=100
$LENGTH=55
$TITLE PASBOD.PAS, last modified 11/28/83, zw
MODULE pasbod OPTIONS SPECIAL(WORD);
(*routine body and statement semantics*)
...
VAR last_stmt_node: tuple; (*last statement mark emitted*)

PROCEDURE stmt_mark;
(*emit a start-statement operator for the start of the statement
  indicated by the current parse tree.  "End" and "until" are also marked.*)
VAR t: tuple;
BEGIN
  NEW(t, start_stmt);
  WITH t^ DO BEGIN
    stmt_kind := ptree^.sym; stmt_source := ptree^.source;
    IF last_stmt_node = NIL THEN stmt_index := 1
    ELSE WITH last_stmt_node^.stmt_source DO
      IF (file_no <> stmt_source.file_no) or

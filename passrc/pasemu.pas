$WIDTH=100
$LENGTH=55
$TITLE PASEMU.PAS, last modified 1/18/84, zw
MODULE pasemu OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- expression manipulation utilities*)
...
PUBLIC VAR with_list: wl_type; (* Initialized to nil in Body. *)
...
(*  Pcomponent is a test function which checks whether a reference is an element
...
begin
  with ref^ do
    pcomponent :=
      ( opcode = substr_ref ) or
      ( not (desc.kind in [records, arrays, strings]) and
	( ((opcode = field_ref) andif base_rec^.desc.base^.packable) or
	  ((opcode = array_ref) andif base_array^.desc.base^.packable) ) ) or
      ( (opcode = ident_ref) andif (id_sym^.kind = values) andif
	(desc.kind = strings) andif (desc.str_kind = nonvarying) andif
	desc.str_flex );
end (* pcomponent *);
$PAGE tag_scan

$WIDTH=100
$LENGTH=55
$TITLE P10MAC.PAS, last modified 1/18/84, zw
MODULE p10mac OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- Macro-10 assembly listing for PDP10 code*)
...
(*sets up the listing file for I/O*)
BEGIN
  IF lf_status = prev_opened THEN fio_reopen(listfb)
  ELSE fio_open(listfb, '.LST ' || list_file);
    ...
    case kind of
      ...
      instruction:
        begin
          ...
          fio_tab (listfb, 25);
          fio_write (listfb, cvoct (inst.acc)); fio_write (listfb, ',          if inst.indirect then fio_write (listfb, '@');

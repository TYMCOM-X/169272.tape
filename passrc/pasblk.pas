$WIDTH=100
$LENGTH=55
$TITLE PASBLK.PAS, last modified 1/10/84, zw
MODULE pasblk OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- block semantics*)
...
PUBLIC VAR env_compilation: BOOLEAN; (* TRUE if compilation is an envmodule *)
...
public procedure close_block;
 ...
 begin
  ...
  with cur_block^ do begin
    IF sw(cur_block^.dump_switches,'ST') THEN BEGIN
      dmpblock(cur_block); dmpstable(cur_block)
    END;
       ...
       then begin                                       (* same name declared in current block *)
         with name^.scopechain^ (* the symbol *) do begin
           if (kind = consts) andif (init_value.kind = subr_cst)        (* is name of actual subr *)
             then chk_forward := init_value.blkp^.forward_dcl
         end
       end
  end;

$WIDTH=100
$LENGTH=55
$TITLE Q10DSC.PAS, last modified 1/18/84, zw
MODULE q10dsc OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- pdp10 quick code generator string and io routines*)
...
PUBLIC FUNCTION fetchstring
  (exp: expr; length_context: str_len_context): str_desc
...
(*frees all of the address descriptors contained within
...
(*takes a string descriptor and returns an address descriptor
  for the byte beginning the text of the string.  Note that
  parameter DESC must be a VAR parameter because the TEXT_VALID
...
BEGIN
  WITH desc DO BEGIN
    (*If the TEXT_ADDR field is already valid then simply duplicate
      and return it.*)
    IF text_valid THEN skip_len_words := duplicate_addr(text_addr)
    ELSE BEGIN
      (*The TEXR field is not valid.  We must increment the base
        address by the length of any bound or length words.  The
        TEXT_ADDR field will then be set.*)
      (*flex string ?*)
      IF base_is_bound THEN increment := 1 ELSE increment := 0;
      (*varying string? *)   
      IF (type_desc.kind = strings) ANDIF (type_desc.str_kind = varying)
      ...
      text_valid := true; text_addr := duplicate_addr(skip_len_words);
...
(*accepts a string descriptor and assures that the length and
  address fields are addressible as required in the runtime argument
...
BEGIN
  WITH str DO BEGIN
    IF (base_addr.mode = byte) ORIF
      (text_valid ANDIF (text_addr.mode = byte)) THEN format := x_format
    ELSE IF type_desc.kind = chars THEN format := c_format
    ELSE format := f_format;
    IF (format = f_format) AND NOT text_valid THEN BEGIN
      taddr := skip_len_words(str); free(taddr)
    END;
    IF len_addr.indirect
    THEN len_addr := reg_addr(load_addr(len_addr, unsigned_value, 36))
  END
END;
...
begin
  with exp^ do begin
    ...
    if use_dynamic_temp then begin
      ...
      with concat_desc do begin
        len_addr := int_value (temp_length);
      end;
    end;
    with concat_desc do begin
...
VAR reg: registers; str_len, temp_addr: addr_desc; temp_desc: str_desc;
BEGIN
  WITH node^.file_arg^ DO BEGIN
    IF opcode = in_str_op THEN BEGIN
      str_file := fetchtranslated(operand[1], actual_length);
      (*utilize trans. routine to make temp. copy*)
      IF NOT is_str_temp(str_file)
      THEN str_file := do_translation(str_file, TRUE);
      getputstring := TRUE;
      str_addr := get_temp(2); str_len := str_addr;
      str_len.offset := str_len.offset + 1;
      temp_addr := skip_len_words(str_file);
      IF temp_addr.mode <> byte THEN BEGIN
        gen_rm(movei, 1, temp_addr); gen_ri(hrli, 1, #o440700);
        gen_rm (movem, 1, str_addr)
      END
      ELSE IF is_register(temp_addr) THEN
        gen_rm (movem, temp_addr.offset, str_addr)
      ELSE BEGIN
        gen_rm(move, 1, temp_addr); gen_rm(movem, 1, str_addr)
      END;
      free(temp_addr); str_bptr := true;
      gen_rm(move, 1, str_file.len_addr); gen_rm(movem, 1, str_len);
    ...
    ELSE IF opcode = out_str_op THEN WITH original_target DO BEGIN
      ...
      (*make room for possible length*)
      IF type_desc.str_flex THEN BEGIN (*use dynamic temp*)
	init_str_desc(str_file);
	str_file.base_addr :=
 	  alc_dynamic_temp(len_addr, chars_per_unit, bits_per_char)
      END
      ELSE str_file := alloc_str_temp(max_len, TRUE);

$PAGE

      IF operand[1]^.desc.str_kind = varying
      THEN str_file.len_addr := duplicate_addr(str_file.base_addr)
      ELSE str_file.len_addr := duplicate_addr(len_addr);
      str_file.len_context := len_context; str_file.type_desc := type_desc;
      IF operand[1]^.desc.str_kind = varying THEN BEGIN
        IF text_valid ANDIF (text_addr.mode = byte) THEN BEGIN
          ...
          IF NOT is_register(text_addr) THEN BEGIN
            gen_rm(move, 1, text_addr); gen_rm(movem, 1, str_addr)
          END
          ELSE gen_rm(movem, text_addr.offset, str_addr)
        END
        ELSE BEGIN
          temp_addr := skip_desc_word(str_file);
          gen_rm(movei, 1, temp_addr); gen_rx(setzm, 0, 1);
          gen_rm(movem, 1, str_addr);
          free(temp_addr); str_bptr := FALSE
        END
      END
      ELSE BEGIN
        temp_addr := skip_desc_word (str_file);
        ...
        ELSE BEGIN
          ...
          gen_rm(movem, 1, str_addr)
        END;
        str_bptr := TRUE; free(temp_addr)
      END
    END
    ELSE BEGIN (*must be to file*)
      reg := nond_load(node^.file_arg, 36);
      decr_reg_usages(reg); getputstring := FALSE
    END
  END
END; (*io_begins*)
...
PROCEDURE free_both;
BEGIN
  free_string(str_file); free_string(original_target)
END;
BEGIN
  WITH node^.file_arg^ DO BEGIN
    IF getputstring THEN BEGIN
      IF opcode = out_str_op THEN BEGIN
        IF operand[1]^.desc.str_kind = nonvarying THEN BEGIN
          gen_rt(pushj, sb, rt_wr_dnn); gen_rm(arg, 0, str_addr)
        END;
	...
	move_string(original_target, str_file, FALSE, free_both, FALSE, TRUE)
      END
    END
    ELSE BEGIN addr := fetch(node^.file_arg); free(addr) END
  END
END;
...
(*generates code for an open, reset, rewrite or update op, and returns
  an address descriptor to the register in which it has been loaded.*)
VAR
...
BEGIN
  WITH node^ DO BEGIN
    IF operand[1] <> NIL THEN iname := 1 ELSE iname := 2;
    ...
    options_set :=
      set_fetch(operand[3], operand[3]^.desc.set_lwb,
      operand[3]^.desc.set_length);
    IF options_set.nargs = 0 THEN BEGIN
      options_addr := reg_addr(get_reg (36));
      gen_ri(movei, options_addr.offset, 0)
    END
    ELSE BEGIN
      make_set(options_set, 0,
        operand[3]^.desc.set_lwb + operand[3]^.desc.set_length, TRUE);
    ...
    IF file_str_desc.text_addr.mode <> byte THEN BEGIN
      ...
      IF file_str_desc.text_addr.indirect THEN BEGIN
        ...
        gen_ri(hrli, reg, #o440700);
        file_str_desc.text_addr := reg_addr(reg)
      END
      ELSE BEGIN
        file_str_desc.text_addr :=
	  increment_addr(file_str_desc.text_addr, -1, 28, 7);
    ...
    END
    ELSE BEGIN
      IF open_mode = binaryfile THEN rts := rt_open_binary
      ELSE rts := rt_open_typed;
      CASE opcode OF
        open_op, reset_op: bits := 8;
        rewrite_op: bits := 4;
        update_op: bits := 12;
	OTHERS: ASSERT(FALSE)
      END;
      IF desc.base^.packable THEN bits := bits + 2
    END;
    IF iname = 2 THEN bits := bits + 1;
    (*call the runtime routine*)
    gen_rt(pushj, sb, rts);
    gen_ri(arg, bits, open_size);
    gen_str_desc(file_str_desc, str_format);
    gen_rm(arg, 0, options_addr);
    free_string(file_str_desc); free(options_addr);
    (*The file control block address is in register 1.
      Move it into some other register, which will be
      the return register for the operation.*)
    reg := get_reg(36); gen_rr(move, reg, 1);

$WIDTH=100
$LENGTH=55
$TITLE Q10EXP.PAS, last modified 1/18/84, zw
MODULE q10exp OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- PDP10 quick code generator exp eval and reg manip*)
...
PUBLIC FUNCTION load_addr
 (addr: addr_desc; align: data_alignment; size: bit_range): registers; FORWARD;
PUBLIC FUNCTION fetch
  (exp: expr): addr_desc OPTIONS SPECIAL(COERCIONS); FORWARD;
...
BEGIN
  FOR reg := MINIMUM(registers) TO MAXIMUM(registers) DO BEGIN
    WITH regdesc[reg] DO BEGIN associate := noreg; uses_remaining := 0 END
  END;
  with_base := #o15; (*withs will be allocated from #o14 down*)
...
(*similar to decr_reg_usages, but is used where the intention
  is to release only one of a pair of registers, not both*)
BEGIN
  IF (reg <> noreg) ANDIF (reg < #o15) THEN WITH regdesc[reg] DO BEGIN
    uses_remaining := uses_remaining - 1;
    ASSERT(uses_remaining <> MAXIMUM(usage_range));
    IF (uses_remaining <= 0) ANDIF (associate <> noreg) THEN BEGIN
      (*should be a symmetric relationship*)
      ASSERT(regdesc[associate].associate <> noreg);
      regdesc [associate].associate := noreg; associate := noreg
    END
  END
END;
...
BEGIN
  regdesc[reg1].associate := reg2; regdesc[reg2].associate := reg1
END;

$PAGE regs_used

PUBLIC FUNCTION regs_used(maddr: addr_desc): registers;
(*returns the register used by an address descriptor, if any*)
BEGIN
  IF (maddr.reloc.kind = register_sc) ANDIF
    ((maddr.mode <> byte) ORIF (maddr.index = 0)) THEN
    regs_used := maddr.offset
  ELSE IF (maddr.index > 1) THEN regs_used := maddr.index
  ELSE regs_used := noreg
END;

$PAGE free

PUBLIC PROCEDURE free(maddr: addr_desc);
(*releases any index register associated with an ADDR_DESC*)
BEGIN
...
begin
  ...
  if maddr.indirect orif
    ...
    gen_rm (movei, with_base, madd* load with-reg ... *)
...
begin
  if is_register ( addr ) then begin            (* address is a register *)
    ...
    if regdesc[reg + 1].uses_remaining = 0 then begin           (* next reg is free, allocate it *)
      ...
      free ( addr );                            (* move value to first reg of pair *)
...
PUBLIC FUNCTION increment_addr
  (base_addr: addr_desc; word_offset: code_offset;
   bits_offset: bit_offset; component_width: bit_range): addr_desc;
(*returns an address descriptor for  a field of the specified width at the
  specified offset from the base address.  The address is transformed in that
  any registers used in BASE ADDR may be freed and BASE ADDR must be regarded
  as invalid upon return.*)
VAR reg: registers;
BEGIN
  ...
  WITH increment_addr DO BEGIN
    (*update the address.  Three cases are distinguished: (1) an address
      of the form @reg is asserted not to happen; (2) all other indirect
      addresses will cause the address to be loaded into a register and an
      address of the form WORD_OFFSET(REG) is generated, (3) all non-indirect
      addresses simply have WORD_OFFSET added to the offset field of the
      instruction.  Note that not all address modes are considered legal
      inputs to this routine.*)
    IF word_offset <> 0 THEN BEGIN
      ASSERT(NOT ((reloc.kind = register_sc) AND indirect) ); (*@reg*)
      IF indirect THEN BEGIN (*other indirect addressses*)
        free(increment_addr); reg := get_reg(bits_per_unit);
        gen_rm(movei, reg, increment_addr);
        increment_addr := absolute_reference;
        index := reg; offset := word_offset
      END
      ELSE offset := offset + word_offset (*non-indirect address*)
    END;
    (*set mode of address descriptor*)
    IF (component_width >= bits_per_unit) OR (component_width = 0)
    THEN mode := fw
    ELSE IF (component_width = 18) ANDIF (bits_offset = 0) THEN mode := lhw
    ELSE IF (component_width = 18) ANDIF (bits_offset = 18) THEN mode := rhw
    ELSE mode := slice;
    (*set slice fields because packed array accesses always use byte ptrs*)
    slice_size := MIN(component_width, bits_per_unit);
    slice_offset := MIN(bits_offset, bits_per_unit - 1)
  END
END;
...
(*returns an address descriptor for the value described by a CST REF
  expression tuple.  The constant NULL LOCATION is returned for subroutine
  constants.*)
BEGIN
  const_addr := null_location;
  WITH exp^.cst_val DO CASE kind OF
    ptr_cst: const_addr := int_value(int_nil);
    scalar_cst: const_addr := int_value(ival);
    subr_cst: WITH const_addr.reloc DO BEGIN
      kind := def_sc; reldef := get_def(subr_def, blkp^.number)
    END;
    OTHERS: BEGIN
      const_addr.reloc := gen_cnode(valp);
      (*if expected a nonvarying constant, but gen_cnode generated
	a length word, then skip over it*)
      IF (kind = string_cst) ANDIF
        ((exp^.desc.str_kind = nonvarying) AND valp^.str_varying_ref)
      THEN const_addr := increment_addr(const_addr, 1, 0, bits_per_unit)
    END
  END
END;
...
(*takes a FIELD REF tuple and returns an address descriptor for the referenced
  field.  The operand of the FIELD REF tuple is evaluated in the process.*)
BEGIN
  WITH field_ref_exp^ DO BEGIN
    WITH field_sym^ DO field_addr := increment_addr(
      fetch(base_rec), (*get base address of record*)
      fld_offset DIV bits_per_unit, (*field address*)
      fld_offset MOD bits_per_unit, (*  "       "  *)
      fld_width);
...
(*generates code for an ADDR_OP expression tuple. An address
  descriptor for the address formed is returned. *)
CONST flex_arr_desc_size = 36; flex_str_desc_size = 36;
VAR string_desc: str_desc; addr: addr_desc;
BEGIN
  WITH exp^ DO BEGIN
    ...
    IF operand[1]^.desc.kind = strings THEN BEGIN
      ...
      IF string_desc.base_is_bound (*if flex, skip bounds word*)
      THEN addr := increment_addr(addr, flex_str_desc_size, 0, bits_per_unit);
      free_string(string_desc)
    END
    ELSE BEGIN
      addr := fetch(operand[1]);
      IF dynamic_flex (operand[1])
      THEN addr := increment_addr (addr, flex_arr_desc_size, 0, bits_per_unit)
    END;
    (*move the address of the operand to a register and return
      the address of the register*)
    ASSERT(NOT is_register (addr)); ASSERT(NOT addr.immediate);
    free(addr);
...
(*is passed an ARRAY_REF tuple corresponding
...
VAR
  ...
  elem_size: bit_range;
  reg: registers;
  words_per_elem: code_address;
  lower_bound: code_address;
BEGIN
  WITH array_ref_exp^ DO BEGIN
    (*this information needed later*)
    ...
    IF index_is_constant THEN free(index_addr);
    ...
    IF array_typ^.generic THEN BEGIN
      IF index_is_constant THEN BEGIN
        ...
        IF words_per_elem <> 1 THEN gen_ri(imuli, reg, words_per_elem);
        ...
        WITH unpacked_array_addr DO BEGIN
          index := reg; offset := const_part * words_per_elem
        END;
        free(base_addr)
      END
      ELSE BEGIN
    ...
    ELSE BEGIN
      ...
      IF index_is_constant THEN BEGIN
        ...
        unpacked_array_addr :=
	  increment_addr(base_addr, const_part, 0, elem_size)
      END
      ELSE BEGIN
        ...
        IF (base_addr.index = 0) ANDIF 
           ...
           (base_addr.reloc.kind <> register_sc )
        THEN unpacked_array_addr := base_addr
        ELSE BEGIN
	  IF base_addr.indirect THEN BEGIN
	    base_addr.indirect := false; gen_rm(add, reg, base_addr)
	  END
	  ELSE gen_rm(addi, reg, base_addr);
          unpacked_array_addr := absolute_reference
        END;
        WITH unpacked_array_addr DO BEGIN
          index := reg; offset := offset - lower_bound * words_per_elem;
          IF dynamic_flex(base_array) (*compensate for upperbound word,*)
  ...
  WITH array_ref_exp^ DO BEGIN
    ...
    IF generic THEN BEGIN
      ...
      IF index_is_constant THEN BEGIN
        ...
        gen(movn, reg, base_addr.index, base_addr.offset - 2, base_addr.reloc)
      END
      ELSE gen(sub, reg, base_addr.index, base_addr.offset-2, base_addr.reloc)
    END;
    ...
    IF const_part <> 0 THEN BEGIN
      ...
      END
    END
    ELSE BEGIN word_offset := 0; bits_offset := 0 END;
    base_addr := increment_addr(base_addr, word_offset, bits_offset,elem_size);
    (*calcuate the elements address.  Unless both the index and the lower
      bound are known at compile time, an ADJBP instruction will be emitted.*)
    IF NOT (index_is_constant ANDIF NOT generic) THEN BEGIN
      ...
      IF base_addr.indirect THEN BEGIN
        ...
        WITH base_addr DO BEGIN
          indirect := FALSE; mode := slice; offset := 0;
          reloc := none; index := base_reg; slice_size := elem_size
        END
      END;
      base_reg := regs_used(base_addr); (*associate any reg used by the byte*)
      IF base_reg <> noreg THEN BEGIN
        IF base_reg < with_base 
        THEN tag_reg(reg, base_reg) (*pointer, with the reg containing*)
                                    (*the byte pointer, so regs referred to*)
                                    (*by byte pointer will be freed*)
        ELSE decr_reg_usages(base_reg) (*with-reg, or stack pointer*)
      END;
      IF prog_options.ki_code_opt THEN BEGIN
	gen_rt(jsr,0,rt_inst_sml); gen(adj_bp, reg, 0, 0, gen_bptr(base_addr)) 
      END
      ELSE gen(adjbp, reg, 0, 0, gen_bptr(base_addr));
      packed_array_addr := reg_addr_desc;
      WITH packed_array_addr DO BEGIN
        mode := byte; offset := reg;
        slice_size := elem_size (*needed for shift of packed sets*)
      END
    END
    ELSE packed_array_addr := base_addr;
...
(*generate an address descriptor for an ARRAY REF or SUBSTR REF tuple.
  The operands of the ARRAY REF tuple are evaluated in the process, thus
  generating any code necessary to make the array elements addressable.*)
VAR
  reg, reg2: registers;
  base_desc: str_desc;
...
BEGIN
  IF array_ref_exp^.opcode = substr_ref THEN WITH array_ref_exp^ DO BEGIN
    ...
    IF base_addr.indirect THEN BEGIN
      ...
      gen_ri(hrli, reg2, #o440700);
      IF prog_options.ki_code_opt THEN BEGIN
	gen_rt(jsr,0,rt_inst_sml); gen_rr (adj_bp, reg, reg2) 
      END
    ...
    ELSE IF (base_addr.mode <> byte) ANDIF (substr_index^.result = nil) ANDIF
      iconstp(substr_index, cst_index_value)
    THEN array_addr := increment_addr(
      base_addr,
      (cst_index_value - 1) DIV 5,
      ((cst_index_value - 1) MOD 5) * 7,
      7)
    ELSE IF base_addr.mode <> byte THEN BEGIN
      ...
      IF prog_options.ki_code_opt THEN BEGIN
	gen_rt(jsr,0,rt_inst_sml); gen_rm (adj_bp, reg, temp_addr) 
      END
      ...
      IF base_reg <> noreg THEN BEGIN
        IF base_reg < with_base THEN tag_reg(reg, base_reg)
        ELSE decr_reg_usages(base_reg)
      END
    END
    ELSE BEGIN
      reg := load(substr_index, bits_per_unit);
      free(base_addr);
      IF prog_options.ki_code_opt THEN BEGIN
	gen_rt(jsr,0,rt_inst_sml); gen_rm(adj_bp, reg, base_addr) 
      END
      ELSE gen_rm(adjbp, reg, base_addr);
      array_addr := reg_addr (reg);
      array_addr.mode := byte
    END;
    IF base_desc.trans_code <> no_trans
    THEN array_addr := case_op(array_addr, base_desc.trans_code)
...
(*passed an expression class tuple.  It calls the necessary routines
  to generate just enough code to make the result of the expression
  addressible on the target machine.  Normall, FETCH or FETCHSTRING
  should be called to generate code for an expression tuple since
  they contain the logic necessary to handle tuples with multiple uses.*)
VAR addr: addr_desc; reg, reg1: registers; string_desc: str_desc;
BEGIN
  ASSERT(exp <> NIL);
  WITH exp^ DO BEGIN
    CASE opcode OF
      cst_ref: do_fetch := const_addr(exp);
      ident_ref: do_fetch := ident_addr(exp);
      field_ref: do_fetch := field_addr(exp);
      ptr_ref: do_fetch := deref_addr(exp);
      array_ref: do_fetch := array_addr(exp);
      buffer_ref: do_fetch := file_deref(exp);
      substr_ref: BEGIN
        ASSERT(desc.kind = chars); do_fetch := array_addr(exp)
      END;
      iadd_op, isub_op, imul_op: do_fetch := single_op_addr(exp);
      idiv_op, imod_op: do_fetch := idiv_or_imod(exp);
      radd_op, rsub_op, rmul_op, rdiv_op: do_fetch := real_op(exp);
      sclcvt_op: do_fetch := do_fetch(operand[1]);
      expii_op: do_fetch := reg_addr(rt_call(exp, rt_exp_ii));
      expri_op: do_fetch := reg_addr(rt_call(exp, rt_exp_ri));
      exprr_op: do_fetch := reg_addr(rt_call(exp, rt_exp_rr));
      ineg_op: do_fetch := reg_addr(do_move_op(movn, exp, bits_per_unit));
      rneg_op: do_fetch := real_unary_ops(exp);
      iabs_op: do_fetch := reg_addr(do_move_op(movm, exp, bits_per_unit));
      rabs_op: do_fetch := real_unary_ops(exp);
      float_op: do_fetch := do_float_op(exp);
      trunc_op: do_fetch := reg_addr(round_or_trunc(exp));
      sqrt_op: do_fetch := reg_addr(rt_call(exp, rt_r_sqrt));
      ln_op: do_fetch := reg_addr(rt_call(exp, rt_r_ln));
      log_op: do_fetch := reg_addr(rt_call(exp, rt_r_log));
      exp_op: do_fetch := reg_addr(rt_call(exp, rt_r_exp));
      sin_op: do_fetch := reg_addr(rt_call(exp, rt_r_sin));
      arcsin_op: do_fetch := reg_addr(rt_call(exp, rt_r_asin));
      sinh_op: do_fetch := reg_addr(rt_call(exp, rt_r_sinh));
      cos_op: do_fetch := reg_addr(rt_call(exp, rt_r_cos));
      arccos_op: do_fetch := reg_addr(rt_call(exp, rt_r_acos));
      cosh_op: do_fetch := reg_addr(rt_call(exp, rt_r_cosh));
      tan_op: do_fetch := reg_addr(rt_call(exp, rt_r_tan));
      tanh_op: do_fetch := reg_addr(rt_call(exp, rt_r_tanh));
      cotan_op: do_fetch := reg_addr(rt_call(exp, rt_r_ctan));
      addr_op: do_fetch := address_op(exp);
...
$PAGE
	...
	END
      ...
      length_op: BEGIN
        ...
        do_fetch := string_desc.len_addr
      END;
      lwc_op, upc_op: BEGIN (*case conversion for chars only*)
        ASSERT(desc.kind = chars); do_fetch := fetch(operand[1]);
        IF opcode = lwc_op
        THEN do_fetch := case_op(do_fetch, lower_trans)
        ELSE do_fetch := case_op(do_fetch, upper_trans);
      END;
      dim_op: do_fetch := do_dim_op(exp);
      round_op: do_fetch := reg_addr(round_or_trunc(exp));
      arctan_op: BEGIN
        IF upperbound(operand) = 1
        THEN do_fetch := reg_addr(rt_call(exp, rt_r_atan))
        ELSE do_fetch := reg_addr(rt_call(exp, rt_r_atn2))
      END;
      imin_op, rmin_op: do_fetch :=
        reg_addr(do_min_max_op(exp, ltc, expr_size(exp^.desc)));
      imax_op, rmax_op: do_fetch :=
        reg_addr(do_min_max_op(exp, gtc, expr_size(exp^.desc)));
      index_op: do_fetch := do_index_op(exp);
      search_op, verify_op: do_fetch := str_search_verify(exp);
      alc_temp_op: do_fetch :=
        get_temp((exp^.desc.base^.base_size + 35) DIV 36);
      agg_val: ASSERT(false); (*fetch should have seen non-nil result field*)

$PAGE

      func_call_op: BEGIN
        IF NOT p_b_address(exp^.desc.base)
	THEN do_fetch := scl_function_call(exp)
        ELSE BEGIN
          do_fetch :=
	    get_temp((exp^.desc.base^.base_size + 35) DIV bits_per_unit);
          pas_call(exp, addr_return_value, nil, do_fetch)
        END
      END;
      ile_op..filne_op, and_op, or_op, in_op, bnot_op, odd_op: do_fetch :=
        reg_addr(load_bool(exp, FALSE));
      new_op, extent_op: BEGIN
        addr := argument(operand[1]); free(addr);
        reg := get_reg(bits_per_unit);
        IF opcode = extent_op
      ...
      END;
      open_op, rewrite_op, reset_op, update_op: do_fetch := rt_open_call(exp);
      eof_op, eoln_op, eopage_op: BEGIN
        do_fetch := absolute_reference;
        do_fetch.index := load(operand[1], bits_per_unit);
        do_fetch.mode := slice; do_fetch.slice_size := 1;
        IF opcode = eof_op THEN do_fetch.slice_offset := 0
        ELSE IF opcode = eoln_op THEN do_fetch.slice_offset := 1
        ELSE IF opcode = eopage_op THEN do_fetch.slice_offset := 2
	ELSE do_fetch.offset := 3
      END;
      filesize_op: BEGIN
        do_fetch := argument(operand[1]);
        gen_rt(pushj, sb, rt_file_size); gen_rm(arg, 0, arg_addr(do_fetch));
        free(do_fetch);
        reg := get_reg(bits_per_unit); gen_rr(move, reg, 1);
        do_fetch := reg_addr(reg)
      END;
      cursor_op: BEGIN
        reg := load(operand[1], bits_per_unit);
        do_fetch := absolute_reference;
        do_fetch.index := reg; do_fetch.offset := 4
      END;
      extstatus_op: BEGIN
        gen_rt(pushj, sb, rt_extstatus);
        reg := get_reg(bits_per_unit);
        gen_rr(move, reg, 1); do_fetch := reg_addr(reg)
      END;
...
$PAGE
      ...
      iostatus_op: BEGIN
        IF operand[1] <> NIL THEN BEGIN
          ...
          gen_rt(pushj, sb, rt_iostatus); gen_rm(arg, 0, arg_addr(addr));
          free(addr)
        END
        ...
        do_fetch := reg_addr(reg)
      END;
      masked_op, pending_op: BEGIN
	reg := get_reg(bits_per_unit); addr := fetch(operand[1]);
	free(addr);
	IF opcode = masked_op
	THEN gen_rt(pushj, sb, rt_masked)
	ELSE gen_rt(pushj, sb, rt_pending);
	gen_rm(arg, reg, addr); do_fetch := reg_addr(reg)
      END;
      mathstatus_op, prgmstatus_op, spclstatus_op, exiostatus_op: BEGIN
	IF opcode = mathstatus_op THEN gen_rt(pushj, sb, rt_stat_math)
	ELSE IF opcode = prgmstatus_op THEN gen_rt(pushj, sb, rt_stat_program)
	ELSE IF opcode = spclstatus_op THEN gen_rt(pushj, sb, rt_stat_special)
	ELSE gen_rt(pushj, sb, rt_stat_exio);
	reg := get_reg(bits_per_unit); gen_rr(move, reg, 1);
	do_fetch := reg_addr(reg)
      END;
      random_op: BEGIN
        IF upperbound(operand) = 1 THEN BEGIN
          addr := fetch(operand[1]); free(addr);
          IF desc.precision > srealprec
          THEN reg := get_reg(2 * bits_per_unit)
          ELSE reg := get_reg(bits_per_unit);
          gen_rt(pushj, sb, rt_rand_set); gen_rm(arg, reg, addr)
        END
        ELSE BEGIN
          IF desc.precision > srealprec
          THEN reg := get_reg(2 * bits_per_unit)
          ELSE reg := get_reg(bits_per_unit);
          gen_rt(pushj, sb, rt_rand); gen_rr(arg, reg, 0)
        END;
        do_fetch := reg_addr(reg);
      END;
...
$PAGE
      ...
      time_op: BEGIN
	gen_rt(pushj, sb, rt_time); reg := get_reg(bits_per_unit);
	gen_rr(move, reg, 1); do_fetch := reg_addr(reg)
      END;
      runtime_op: BEGIN
	gen_rt(pushj, sb, rt_runtime); reg := get_reg(bits_per_unit);
	gen_rr(move, reg, 1); do_fetch := reg_addr(reg)
      END;
      OTHERS: BEGIN
        ASSERT(FALSE); do_fetch := reg_addr(get_reg(bits_per_unit))
      END
    END
  END
END;
...
PUBLIC FUNCTION fetch(exp: expr): addr_desc OPTIONS SPECIAL(COERCIONS);
(*passed an expression class tuple and returns an address
...
BEGIN
  ...
  WITH exp^ DO BEGIN
    ...
    IF result <> NIL THEN BEGIN
      addr_ref := PTR(ORD(result)); fetch := addr_ref^
    END
    ...
    IF (usage_count > 1) ANDIF (result = NIL) THEN BEGIN (*first of mult uses*)
      NEW(addr_ref); addr_ref^ := fetch;
    ...
    THEN DISPOSE(result); (*last of multiple uses*)
  END
END;

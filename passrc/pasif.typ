(*PASIF.TYP, last modified 10/28/83, zw*)
$IFNOT pasiftyp
...
$PAGE expressions, IF
	...
	ieq_op, ine_op, rle_op, rlt_op, rgt_op, rge_op, req_op, rne_op, sle_op,
	  slt_op, sgt_op, sge_op, seq_op, sne_op, setle_op, setge_op, seteq_op,
	    setne_op, ptreq_op, ptrne_op, fileq_op, filne_op, or_op, and_op,
	      orif_op, andif_op, in_op, diff_op, both_op, str_comp_op,
	      (* Normal Unary Operators *)
	      ineg_op, rneg_op, iabs_op, rabs_op, bnot_op, sclcvt_op, strcvt_op,
		setcvt_op, float_op, trunc_op, odd_op, sqrt_op, ln_op, log_op,
		  exp_op, sin_op, arcsin_op, sinh_op, cos_op, arccos_op, cosh_op
		    , tan_op, tanh_op, cotan_op, length_op, lwc_op, upc_op,
		      lwb_op, upb_op, dim_op, extent_op,
		      (* Normal N-ary Operators *)
		      gen_set_op, round_op, arctan_op, imin_op, rmin_op, imax_op
			, rmax_op, index_op, verify_op, search_op, agg_val,
			  cat_op, union_op,
			  (* Special Binary Operators *)
			  desc_ref, subr_var_op, gen_andif_op, gen_orif_op,
			  (* Special Unary Operators *)
			  new_op, alc_temp_op, addr_op, in_str_op, out_str_op,
			    io_var_str_op, io_fix_str_op, eoln_op, eof_op,
			      eopage_op, cursor_op, filesize_op, iostatus_op,
				masked_op, pending_op,
				(* Special N-ary Operators *)
				random_op, set_op, extstatus_op, date_op,
				  time_op, runtime_op, mathstatus_op,
				    prgmstatus_op, spclstatus_op, exiostatus_op,
...
$PAGE statements, IF
				      (* S T A T E M E N T S *)
				      (* ------------------- *)
				      call_op, start_stmt, start_with, end_with,
					eval_op, assign_op, dispose_op, nop,
					  abort_op, case_abort_op, start_block,
					    end_block, label_node, jump_op,
					      retjump_op, jump_t_op, jump_f_op,
						jump_in_op, jump_cond_op,
						  gen_jump_op, hndlr_jump_op,
						    case_jump_op, stop_op,
						      return_op, goto_op,
							set_handler_op,
							  rst_handler_op,
							    signal_op,
																			  ...
																			  ,
																				...
																				seek_op
																				  ,
																					...
																					;
$PAGE operator class sub-sets, IF
...
$PAGE tuple_node - miscellaneous statements
    tuple_node = packed RECORD
      prev, next: tuple; (*linear chain of operators ick*)
	...
	(
	  ...
	  first_label, last_label: tuple; (*first and last label nodes
					       in the block*)
	...
	(
	  ...
	  stmt_source: source_id ); (*location in text of stmt, debugger*)
...
$PAGE tuple_node - flow control statements
	...
	(
	  ...
	  low_cntrl, high_cntrl: int_type ); (*range of values of
			computation causing jump*)
...
$PAGE tuple_node - data reference expressions
	...
	(
	  ...
	  blk_input_tuple: boolean; (*true if the tuple is a basic block
		       input*)
	  ...
	  usage_count: usage_range; (*number of references to this node in
			a basic block*)
	  CASE tuple_opcodes OF
	    ...
	    (
	      ...
	      field_sym: sym ); (*symbol table field record with
			   this name*)
	    ...
	    (
	      base_array: expr; (*array value being indexed;
	      is another array_ref,  when multiple dimensions are used*)
	      index_val: expr );
	    substr_ref: (*substring reference*)
	    (
	      base_string: expr; (*the string being selected from*)
	      substr_index: expr; (*the index into the string*)
	      substr_length: expr ); (*the length to extract;
			    nil => remainder*)
	    ...
	    (
	      nlevels: 0..63 ); (*0 is stack of current,
		    1 is parent, etc.*)

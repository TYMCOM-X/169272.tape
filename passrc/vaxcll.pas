$WIDTH=100
$LENGTH=55
$TITLE VAXCLL.PAS, last modified 1/18/84, zw
MODULE vaxcll OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- VAX subroutine call utilities*)
...
begin (* pas_call *)
  ...
  if ( node^.subr^.opcode <> cst_ref ) and
     ( (node^.subr^.opcode <> ident_ref) orif
       (node^.subr^.id_sym^.kind <> consts) ) then begin (* be subr var *)

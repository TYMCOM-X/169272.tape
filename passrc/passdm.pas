$WIDTH=100
$LENGTH=55
$TITLE PASSDM.PAS, last modified 1/12/84, zw
MODULE passdm OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- symbol table dump and xref facility*)
...
(* REVERSE reverses the links in a singly linked list of reference nodes.
   A good general-purpose algorithm.  Watchfully now... *)
...
begin
  fio_line (fb, version || ' Symbol Table Dump   ' ||
...
begin
  ...
  while s <> nil do begin
    if s^.name <> nil then begin
      s^.scopechain := s^.name^.scopechain;  (* preserve current top *)

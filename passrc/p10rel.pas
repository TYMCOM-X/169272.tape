$WIDTH=100
$LENGTH=55
$TITLE P10REL.PAS, last modified 1/18/84, zw
MODULE p10rel OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- PDP10 relocatable file emission*)
...
procedure ate ( value: code_address; rel: rel_syllable;
                     lr: left_right; ic: code_address;
                     var rslt_value: code_address; var rslt_rel: boolean );
...
begin
  ...
  if root_block^.children^.kind = program_blk then begin
    ...
    if prog_options.ki_code_opt then rel_sixbit ('PAILIB') 
    else rel_sixbit ('PASLIB');
    prgm_ppn (libdir);
    foo := pdr('foo'); (*just so we wont get that silly warning message*)

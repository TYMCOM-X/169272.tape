$WIDTH=100
$LENGTH=55
$TITLE VAXREL.PAS, last modified 1/10/84, zw
MODULE vaxrel OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- VAX relocatable file generator*)
...
(*opens the relocatable binary output file and emits those records
  which should be emitted at the start of the file, and which do not require
  processing of the intermediate form.*)
VAR compilation_date: dtime_ext; j: 1 .. 34; rt: rt_first .. rt_last_used;
BEGIN
  IF rel_file <> '' THEN BEGIN  
    REWRITE(relf, '.' || rel_extension || '[,]' || rel_file);
    IF NOT EOF(relf) THEN BEGIN
      ttymsg('?unable to write rel file: ' || rel_file);
...
$ENDIF
  ...
  FOR j := 1 TO 7 (*creation date/time*)
  DO rel_byte(ORD(compilation_date[j])); (*dd-mmm-*)
  rel_byte(ORD('1')); rel_byte(ORD('9'));
  FOR j := 8 TO 15
  DO rel_byte(ORD(compilation_date[j])); (*yy hh:mm*)
  FOR j := 1 TO 17 (*last_patch date/time*)
  DO rel_byte(ORD(' '));
  rel_record(obj$c_hdr); (*second header record*)
  rel_byte(1); (*hdr type = lnm*)
  FOR j := 1 TO MIN(33, LENGTH(version))
  DO rel_byte(ORD(version[j]));
  FOR rt := rt_first TO rt_last_used
  DO rt_refs[rt] := FALSE; (*clear table of runtime syms. referenced*)
...
$IF OPSTATS
    ...
    if sw( cur_block^.dump_switches, 'OPSTATS' ) and write_next_byte then begin
...
$IF OPSTATS
    ...
    if sw( cur_block^.dump_switches, 'OPSTATS' ) then begin
...
$IF OPSTATS
	...
	IF sw( cur_block^.dump_switches, 'OPSTATS') THEN BEGIN
	  WRITELN(ops_file); WRITE(ops_file, 'B');
	end;
$ENDIF
...
$IF OPSTATS
	  if sw( cur_block^.dump_switches, 'OPSTATS' ) then begin
	    int_bytes.int := reloc.reldef^.addr - 2 - ic;
	    (* make sure 2nd byte gets written *)
...
$IF OPSTATS
	  ...
	  if sw(cur_block^.dump_switches, 'OPSTATS') and

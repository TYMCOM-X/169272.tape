$WIDTH=100
$LENGTH=55
$TITLE PASALC.PAS, last modified 1/18/84, zw
MODULE pasalc OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- storage allocation module*)
...
EXTERNAL PROCEDURE dmpstorage; (*where is this defined??*)
...
EXTERNAL VAR alc_tables: env; (*where is this defined??*)

PUBLIC PROCEDURE alc_init OPTIONS SPECIAL(COERCIONS);
(*initialize storage allocation tables from heap*)
BEGIN
  getenv(ORD(ADDRESS(alc_tbl_start)), ORD(ADDRESS(alc_tbl_end)), alc_tables)
END;
...
begin
  ...
  while block <> nil do begin
	...
	while call_list <> nil do begin
      ...
      end (* if kind in [program_bubr_blk] *);
      last_block := block;
      block := downward_call_thread;
    end;
  end (* while block <> nil *);
...
(*assigns an address to each condition, variable, and
...
BEGIN
  FOR area := MINIMUM(static_areas) TO MAXIMUM(static_areas) 
  DO counter[area] := alc_area_descs[area].init_offset;
  allocate_all_symbols;
  (*Allocation is now complete, unless quick block analysis is in effect.*)
  IF qbl_allowed AND (qblocks_opt IN all_opts)
  THEN compute_block_offsets;
  ...
  IF sw(prog_options.dump_switches, 'STORAGE')
  THEN dmpstorage
END.
3@V
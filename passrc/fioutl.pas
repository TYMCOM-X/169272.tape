$TITLE FIOUTL.PAS, last modified 4/9/84, zw
MODULE fioutl;
...
PUBLIC PROCEDURE dpymsg(msg: STRING[*]);
...
BEGIN
  rdyio; dpymsg(msg); IF byemsg <> '' THEN dpymsg(byemsg); STOP
END;
...
PUBLIC PROCEDURE delfil(name: file_name);
VAR f: text_file;
BEGIN
  RESET(f, name);
  IF IOSTATUS = IO_OK THEN SCRATCH(f)
END;
...
PUBLIC FUNCTION opnfil
  (VAR fil: TEXT; nam: FILE_NAME; mode: open_mode): BOOLEAN;
...
VAR mode: ARRAY [BOOLEAN] OF open_mode;
BEGIN
  ttyok(FALSE);
  clsout;
...
NPUT := file_stack^.fil; input_is_open := INPUT <> NILF;
...
PUBLIC PROCEDURE dpyfil(name: file_name);
(*display text file to terminal*)
VAR line: STRING[80]; save_output: text_file;
BEGIN
  assume(io_is_ready, '?Terminal I/O not ready.');
  save_output := OUTPUT;
  OUTPUT := TTYOUTPUT;
  IF pushin(name) THEN BEGIN
    WHILE rdlin(line) DO wrlin(line);
    ASSUME(popin, '?Can not close display file.');
  END;
  OUTPUT := save_output
END.
   PXF*r
$WIDTH=100
$LENGTH=55
$TITLE PASOPT.PAS, last modified 1/10/84, zw
MODULE pasopt OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- intermediate form optimization*)
...
$PAGE propagate - in do_summary_analysis
...
var
    s: index_range;
    count: index_range;
$PAGE merge_sets - in propagate - in do_summary_analysis
...
$PAGE dfs - in propagate - in do_summary_analysis
...
$PAGE propagate - main routine - in do_summary_analysis
...
$PAGE do_summary_analysis - main routine

VAR i: index_range; v: vl_link; b: blk; c: call_link;
  classmod, classuse: svector;
BEGIN
  IF sw(prog_options.dump_switches, 'VLSYMS')
  THEN dmpvlsyms;
  ...
  FOR i := 0 TO max_level DO clr_set(vv, i);
  v := vl_list;
  WHILE v <> NIL DO BEGIN
    ...
    v := v^.last
  END;
  IF sw(prog_options.dump_switches, 'VV')
  THEN dmpset(vv, sym_vl_number, 'VARIABLES DECLARED AT LEVELS');
  ...
  FOR i := 0 TO blk_number DO BEGIN
    clr_set(smod, i); clr_set(suse, i)
  END;
  sum_flow(smod, suse);
  IF sw(prog_options.dump_switches, 'USEMOD0') THEN BEGIN
  ...
  b := lex_block; i := max_level + 1;
  WHILE b <> NIL DO BEGIN
    IF i <> b^.level THEN BEGIN
      i := b^.level;
      mov_set(vv, i, smod, 0); mov_set(vv, i, suse, 0)
    END;
    subtract(smod, 0, b^.number); subtract(suse, 0, b^.number);
    b := b^.lex_thread
  END;
  IF sw(prog_options.dump_switches, 'USEMOD1') THEN BEGIN
  ...
  FOR 1 TO max_level DO propagate(smod, suse, i);
  IF sw(prog_options.dump_switches, 'USEMOD2') THEN BEGIN
  ...
  IF root_block^.peer <> NIL THEN BEGIN (*There are classes defined.*)
    ...
    FOR i := 0 TO blk_number DO BEGIN
      clr_set(classmod, i); clr_set(classuse, i)
    END;
    b := root_block^.peer;
    WHILE b <> NIL DO BEGIN (*Compute CLASSMOD(b) and CLASSUSE(b).*)
      ...
      WHILE c <> NIL DO BEGIN (*Process each subroutine called by b.*)
        ...
        mov_set(smod, i, classmod, 0); union(classmod, 0, i);
        mov_set(suse, i, classuse, 0); union(classuse, 0, i);
        c := c^.rlink
      END;
      b := b^.peer
    END;
    ...
    IF sw(prog_options.dump_switches, 'USEMOD3') THEN BEGIN
    ...
    FOR i := 1 TO blk_number DO BEGIN
      mov_set(classmod, i, smod, 0); union(smod, 0, i);
      mov_set(classuse, i, suse, 0); union(suse, 0, i)
    END;
    del_svector(classmod); del_svector(classuse)
  END; (*root_block^.peer <> nil*)
  clr_set(smod, 0); clr_set(suse, 0);
  IF sw (prog_options.dump_switches, 'USEMOD') THEN BEGIN
  ...
  del_svector(visited); del_svector(stacked);
  DISPOSE(dfnumber); DISPOSE(lowlink); DISPOSE(sccroot)
...
$PAGE distinct_indices - in circumscribes
...
var
    ...
    a_cst, b_cst: int_type; (* The constant term from Ia/Ib. *)
...
$PAGE circumscribes - main routine
...
$PAGE cse hash table declarations
...
$PAGE cse_init (cse hash table routines)
...
$PAGE cse_clear (cse hash table routines)
...
$PAGE new_value & new_val_num (cse hash table routines)
...
$PAGE change_entry (cse hash table routines)
...
$PAGE lookup_tuple (cse hash table routines)
...
$PAGE modification (cse hash table routines)
...
$PAGE check_mod (cse hash table routines)
...
$PAGE check_io (cse hash table routines)
...
$PAGE rhs reference list declarations
...
$PAGE rhs_ref_init (rhs reference list routines)
...
$PAGE rhs_ref_clear (rhs reference list routines)
...
$PAGE note_use (rhs reference list routines)
...
var entry: ref_desc;
$PAGE record_components - in note_use
...
$PAGE kill_reference (rhs reference list routines)
...
$PAGE kill_mod_references (rhs reference list routines) 
...
$PAGE kill_idef (rhs reference list routines)
...
$PAGE mod_or_uses - in chk_overlap
...
$PAGE mark_base_input_tuples - in reduce
...
$PAGE fre_hash - in build_fre_table
...
$PAGE fre_match - in build_fre_table
...
$PAGE make_fre - in build_fre_table
...
$PAGE fre_number - in build_fre_table
...
$PAGE create_list - in build_fre_table
...
$PAGE mark_all_references - in build_fre_table
...
$PAGE convert_list_to_table - in build_fre_table
...
$PAGE compute_impl_mod - in build_fre_table
...
$PAGE file_fre - in build_fre_table
...
$PAGE compute_input_output_sets - in build_fre_table
...
$PAGE build_fre_table - main routine

VAR index: hash_index; f: sym_link;
BEGIN
  n_fre := 0; chain := NIL; fre_files := NIL;
  (*Clear the FRE hash table.*)
  FOR index := MINIMUM(index) TO MAXIMUM(index) DO table[index] := NIL;
  create_list;
  mark_all_references;
  convert_list_to_table;
  IF sw(cur_block^.dump_switches, 'FRES') THEN dmpfrtable(fre_table, n_fre);
  compute_impl_mod;
  IF sw(cur_block^.dump_switches, 'IMPL_MOD') 
  THEN dmpset(impl_mod, n_fre, 'IMPL_MOD SETS FOR BLOCK $');
  compute_input_output_sets;
  WHILE fre_files <> NIL DO BEGIN
    f := fre_files^.next; DISPOSE(fre_files); fre_files := f
  END;
  IF sw(cur_block^.dump_switches, 'IOSETS') THEN BEGIN
    dmpset(input_set, n_fre, 'BASIC BLOCK INPUT FRE''S FOR BLOCK $');
    dmpset(output_set, n_fre, 'BASIC BLOCK OUTPUT FRE''S FOR BLOCK $')
  END;
  IF sw(cur_block^.dump_switches, 'IFM_FRE')
  THEN dmptuples('IF FOR BLOCK $, WITH FORMAL REFERENCE EXPRESSIONS')
END; (*build_fre_table*)
...
100 (* not reducible *):
  ...
  IF sw(cur_block^.dump_switches, 'DOM')
  THEN dmpdominators(idom, dom_son, dom_brother, reducible);
...
$PAGE node_idef - in compute_idef
...
$PAGE compute_ew - in node_idef - in compute_idef
...
$PAGE condense - in node_idef - in compute_idef
...
$PAGE searchc - in condense - in node_idef - in compute_idef
...
$PAGE condense - main routine - in node_idef - in compute_idef
...
$PAGE process_component - in node_idef - in compute_idef
...
$PAGE node_idef - main routine - in compute_idef
...
$PAGE compute_idef - main routine
...
var
    ws: ^ stack_array;
$PAGE weak_val - in compute_weak_environment
...
$PAGE compute_weak_environment - main routine
...
begin
  ...
  DISPOSE(root_entry); DISPOSE(ws); del_svector(r_set);
  IF sw(cur_block^.dump_switches, 'WENV')
  THEN dmpwenv(input_tuples, n_blocks);
  FOR i := 1 TO n_blocks
  DO WHILE input_tuples^[i] <> NIL DO BEGIN
...
var
    ...
    file_tpl: tuple;
$PAGE red_gbl_blk - in reduce_global
...
$PAGE reduce_global - main routine
...
begin
  ...
  IF sw(cur_block^.dump_switches, 'FGRAPH')
  THEN dmpfgraph(block_labels, n_blocks);
  (*Reduce the basic blocks of the subroutine to DAGs.*)
  reduce_blocks;
  IF sw(cur_block^.dump_switches, 'DAGS')
  THEN dmptuples('BLOCK $ AFTER BASIC BLOCK OPTIMIZATION');
  (*Compute the dominator tree for the flow graph, representing it in the
    arrays Idom, DomSon, and DomBrother.*)
  make_dominator_tree;
  IF sw(cur_block^.dump_switches, 'NOGLOBAL') THEN RETURN;
  (*Compute the Formal Reference Expressions for this subroutine.*)
  build_fre_table;
  (*Compute the IDEF function for the flow graph.*)
  compute_idef (n_fre);
  IF sw(cur_block^.dump_switches, 'IDEF')
  THEN dmpset(idef, n_fre, 'IDEF FOR BLOCK $');
  IF sw(cur_block^.dump_switches, 'R')
  THEN dmpset(r_set, n_fre, 'INTERMEDIATE SET R FOR BLOCK $');
  compute_weak_environment; reduce_global;
  del_svector(input_set); del_svector(output_set);
  DISPOSE(fre_table); del_svector(idef)
END; (*optimize*)
...
$PAGE low_red_blk - in low_reduce
...
$PAGE do_reference_allocation - in low_reduce
...
$PAGE note_reference - in do_reference_allocation - in low_reduce
...
$PAGE process_loop - in do_reference_allocation - in low_reduce
...
$PAGE loop_search - in do_reference_allocation - in low_reduce
...
$PAGE do_reference_allocation - main routine - in low_reduce
...
begin
  ...
  DISPOSE(ff);
  IF sw(cur_block^.dump_switches, 'LOOPS')
  THEN dmpset(loop_set, n_blocks, 'LOOPS HEADED BY BASIC BLOCKS IN BLOCK $');
  marked := new_svector(0, n_blocks); clr_set(marked, 0);
  NEW(ref_lists, n_blocks);
  FOR i := 1 TO n_blocks DO ref_lists^[i] := NIL;
  NEW(loop_head, n_blocks); loop_depth := 0;
  FOR i := 1 TO n_blocks
  DO IF NOT in_set(marked, 0, i) ANDIF NOT is_empty(loop_set, i)
  THEN process_loop(i);
  IF sw(cur_block^.dump_switches, 'REFS')
  THEN dmprefcounts(ref_lists, n_blocks);
  del_svector(loop_set); DISPOSE(loop_head); DISPOSE(ref_lists)
END; (*do_reference_allocation*)

*****************************************************************************)

$PAGE low_reduce - main routine

VAR index: id_range; b: index_range; dom_last: id_range;
BEGIN
  (*First, clear the hash table.*)
  FOR index := 0 TO hash_limit DO table [index] := NIL;
  val_number := 0;
  ...
  LOOP
    LOOP
      ...
      b := dom_son^[b]
    END;
    WHILE (dom_brother^[b] = 0) AND (idom^[b] <> 0) DO b := idom^[b];
    EXIT IF dom_brother^[b] = 0;
    b := dom_brother^[b];
    dom_last := block_labels^[idom^[b]+1]^.nodeid;
    FOR index := 0 TO hash_limit
    DO WHILE (table[index] <> NIL) ANDIF (table[index]^.nodeid > dom_last)
    DO table[index] := table[index]^.result
  END;
  reclaim;
  (**do_reference_allocation;**)
  (*Dispose of the miscellaneous storage the optimizer has allocated and
    left lying around.*)
  DISPOSE(idom); DISPOSE(dom_son); DISPOSE(dom_brother); DISPOSE(block_labels)
END. (*low_reduce*)

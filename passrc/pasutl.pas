$WIDTH=100
$LENGTH=55
$TITLE PASUTL.PAS, last modified 1/3/84, zw
MODULE pasutl OPTIONS SPECIAL(WORD);
(* symbol table utility routines*)
...
$PAGE new_type
(* NEW_TYPE returnspe node of the specified kind, which is not
   linked into any of the symbol table data structures yet.  *)
...
begin
  ...
  with new_type^ do begin
    ...
    case kind of
      ...
      chars:
          base_type := nil;
      reals: BEGIN precision := 1; rminval := 0.0; rmaxval := 0.0 END;
      bools,
      scalars:
        begin
          base_type := nil;
      ...
      files:
          component_type := nil;
      strings: BEGIN str_kind := varying; str_length := 0 END;
      records:
        begin
          field_list := nil;
          variant_tag := nil;
...
(*  NEW PROC returns a procedure or function type node, with a specified
    number of parameter type slots.  *)

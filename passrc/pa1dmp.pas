$WIDTH=100
$LENGTH=55
$TITLE PA1DMP.PAS, last modified 1/10/84, zw
MODULE pa1dmp OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- pass 1 dump routines*)
...
begin
	...
	swtch := dump_switches;
	while swtch <> nil do begin
	    if swtch <> dump_switches then fio_write (dumpfb, ', ');
	    fio_write (dumpfb, swtch^.nam);
	    if swtch^.ena then fio_write (dumpfb, '!');
	    swtch := swtch^.nxt
	end;

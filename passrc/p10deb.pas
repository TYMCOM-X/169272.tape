$WIDTH=100
$LENGTH=55
$TITLE P10DEB.PAS, last modified 1/18/84, zw
MODULE p10deb OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- PDP10 debug symbol table dumper*)
      ...
      if pages^.following_page <> nil then begin        (* stmt blocks emitted for file *)
        ...
        while page <> nil do with page^ do begin
          if left_page <> nil then begin        (* stmts in page *)
            ...
            (* emit only initial alphanumericion of subtitle *)
...
$PAGE sym_size, value_size, typ_size, blk_size, nam_size
  ...
  begin
    with s^ do
      case kind of
	fields:		sym_size := size (deb_symbol_node, fields);
...
(*relocates pointers within a "type_node" *)
VAR t_size: int_type;
BEGIN

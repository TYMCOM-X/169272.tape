$WIDTH=100
$LENGTH=55
$TITLE VAXGEN.PAS, last modified 1/18/84, zw
MODULE vaxgen OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- VAX quick code generator*)
...
(* BB_END does processing necessary at the end of a basic block.
   ...
   to INSTRUCTION (it was NULLCODE).  If tBEL_NODE for the
...
procedure compile_body ( var begin_addr: def )  options special(coercions);
 ...
 begin
  ...
  if sw(cur_block^.dump_switches, 'IFM0') then
  ...
  if sw(cur_block^.dump_switches, 'IFM') then

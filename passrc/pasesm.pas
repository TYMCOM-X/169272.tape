$WIDTH=100
$LENGTH=55
$TITLE PASESM.PAS, last modified 11/28/83, zw
MODULE pasesm OPTIONS SPECIAL(WORD);
...
(*  RecordSemantics is called with a parse node for a <paren expr> and with
    a type node for a record type returns an AGG_VAL expression tree
...
begin
  ...
  if (list_size >= array_size) orif (array_levels = 1) then
    array_semantics := build_array ( array_levels, def, list_size, trgt_type )
  else begin

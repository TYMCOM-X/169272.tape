$WIDTH=100
$LENGTH=55
$TITLE P10DSC, last modified 1/18/84, zw
MODULE p10dsc OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- PDP10 string processing code generation*)
...
(* FETCH STRING generates a runtime addressing descriptor for a string operand.
   ...
   loads both with "movei rcword" so that the values can be, in effect,
...
public procedure str_index_op ( treg: reg_selector; node: expr );
 ...
 begin
  with node^ do begin
    ...
    gen_rt (pushj, sb, cs_index [str_format (operand[1]), str_format (operand[2])]);
    gen_str_desc (addr1, len1);
    gen_str_desc (addr2, len2);

$WIDTH=100
$LENGTH=55
$TITLE P10EXP.PAS, last modified 1/18/84, zw
MODULE p10exp OPTIONS SPECIAL(WORD);
(*TYM-Pascal compiler -- PDP10 expression evaluation, register manipulation*)
...
(* FREE REG releases a register group for future allocation.  The registers to
   ...
   fields describing the value present are noet, as the value may remain
...
begin
  ...
  LOOP
    ...
    EXIT IF (reg + rs[reg].size) > MAXIMUM(registers);
    reg := reg + rs[reg].size;
  end;
end;

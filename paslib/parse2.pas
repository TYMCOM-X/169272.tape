...
PROGRAM parser;
(*LL(1) parser for regular, context-free grammars, with error recovery*)
...
its production strings.  A start set of a symbol is the set of all terminal
symbols which could ultimatly begin one of its productions, or the productions
of any non-terminals which begin one of its productions.  The start set is
non-trivial only if the symbol is non-terminal, otherwise it is just the
set of itself.  The omit set is a set of all non-terminal which have a null
production string, and thus could be omitted from any production string they
...
(*
If the first token is not in the start set of the start symbol then error,
otherwise, parse the start symbol of the grammar.  The assumption is that the
current token can always be parsed successfully.  To parse a terminal symbol,
check to see if it matches the current token, if so, advance to the next token
...
production strings.  Note that an error check is done before any attempt is
made to parse a symbol of a production string.  The production strings are
tried one after another until either the parse-ok flag is set or an error is
detected.
*)

$PAGE description of error detection algorithm

(*
If the next token is in the lookup set of the current symbol of the current
production string or if the lookup set is null then no error.  Otherwise, if
the next symbol of the production string can be omitted, advance to the next
production symbol and loop.  IF the next symbol of the production could not
be omitted, then there is no way the next token could be parsed and an error
is declared.  Note that there is an assumption that the symbol about to be
parsed could be parsed successfully.
*)

$PAGE description of error recovery algorithm
...
BEGIN (*load production string from INPUT file*)
  IF tkn <> '(' THEN sgnl_err('lookin "(": ' || tkn)
  ELSE BEGIN
    scan;
...
$PAGE generate omit, start and lookup sets

PROCEDURE gen_omit_set(VAR sym_set: sym_set_typ);
VAR sym: sym_typ; prod: prod_typ;
BEGIN (*generate set of tokens which may be omitted*)
  sym_set := nul_set;
  FOR sym := MINIMUM(sym) TO MAXIMUM(sym) DO BEGIN
    prod := grmr.prods[sym];
    WHILE prod <> NIL DO BEGIN
      EXIT IF prod^.str = NIL DO sym_set[sym] := TRUE;
      prod := prod^.next
    END
  END;
  IF dbg THEN BEGIN d('omit set:'); dpy_set(sym_set) END
END;

PROCEDURE gen_start_set(VAR sym_set: sym_set_typ; sym: sym_typ);
  PROCEDURE mak_start_set(sym: sym_typ);
  VAR prod: prod_typ;
  BEGIN (*make start symbol set for given symbol*)
    prod := grmr.prods[sym];
    IF grmr.terms[sym] THEN sym_set[sym] := TRUE
    ELSE IF grmr.non_terms[sym] THEN BEGIN
      IF prod = NIL THEN sgnl_err('grammar broken: ' || sym_nam[sym])
      ELSE REPEAT
        IF prod^.str <> NIL THEN mak_start_set(prod^.str^.sym);
        prod := prod^.next
      UNTIL prod = NIL
    END
  END;
BEGIN (*generate start symbol set for given symbol*)
  sym_set := nul_set; mak_start_set(sym);
  IF dbg THEN BEGIN
    d('start set for: ' || sym_nam[sym]); dpy_set(sym_set)
  END
END;

PROCEDURE gen_lkp_set(VAR sym_set: sym_set_typ; prod_str: str_typ);
BEGIN (*generate lookup set for symbol in production string*)
  IF prod_str = NIL THEN sgnl_err('no symbol for lookup set')
  ELSE BEGIN
    IF prod_str^.next = NIL THEN sym_set := nul_set
    ELSE gen_start_set(sym_set, prod_str^.next^.sym);
    IF dbg THEN BEGIN
      d('lookup set for: ' || sym_nam[prod_str^.sym]); dpy_set(sym_set)
    END
  END
END;
...
BEGIN (*check grammar for validity*)
  ...
  gen_omit_set(sym_set);
  IF sym_set[grmr.start] THEN sgnl_err('start symbol can be omitted');
  gen_trm_set(sym_set);
  FOR sym := MINIMUM(sym) TO MAXIMUM(sym)
...
PROCEDURE chk_str;
VAR sym_set: sym_set_typ; sym: sym_typ;
BEGIN (*check string for validity*)
  IF str = NIL THEN sym := eos_sym ELSE sym := str^.sym;
  gen_start_set(sym_set, grmr.start);
  IF NOT sym_set[sym] THEN sgnl_err('bad first token: ' || sym_nam[sym])
END;
...
$PAGE error recovery algorithm

PROCEDURE fix_err;
BEGIN (*try to fix error*)
  d('fixing error');
  err := FALSE;
  sgnl_err('can not recover from error')
END;

$PAGE error detection algorithm

PROCEDURE err_chk(prod_str: str_typ);
VAR lkp_set, omit_set: sym_set_typ; str: str_typ;
BEGIN (*declare error if next token could not be parsed*)
  d('check: ' || sym_nam[prod_str^.sym]);
  d('next token: ' || sym_nam[next_tkn]);
  str := prod_str; gen_omit_set(omit_set); gen_lkp_set(lkp_set, str);
  (*if str^.next is not NIL then lkp_set <> nul_set*)
  WHILE NOT (err OR (str^.next = NIL) OR lkp_set[next_tkn]) DO BEGIN
    IF omit_set[str^.next^.sym] THEN BEGIN
      str := str^.next; gen_lkp_set(lkp_set, str)
    END
    ELSE BEGIN
      sgnl_err('about to parse: ' || sym_nam[prod_str^.sym]);
      sgnl_err('bad token: ' || sym_nam[next_tkn])
    END
  END;
  IF NOT err THEN d('could parse next token')
END;

...
public procedure store_reg (reg: registers; mem: addr_desc);
 begin
  if regdesc[reg]^.value_size = 2
    then gen_rm (dmovem, reg, mem)
  else if regdesc[reg]^.loc.mode = rhw
    then begin
      if regdesc[reg]^.signed
        then gen_rm (hrrem, reg, mem)
...
(* SAVE REG insures that the contents of a register can be retrieved from
   memory at some later point in the program.  If the value is not an (immediate)
   constant, and if it has not been previously stored, then value is assigned
   a temporary location and a non-mandatory store emitted.  The store can be
   made mandatory by marking the location of the value as "allocated". *)
...
procedure save_reg (sreg: registers);
 ...
 begin
  vdesc := regdesc[sreg]^.contents;     (* see if anything is in the register *)
  if vdesc = nil then return;
  ...
  with vdesc^ do begin
    if loc.reloc.kind = unallocated then begin
      loc := temp_reference;
      if (not internal) andif (op^.opcode = display_op) then
	loc.mode := rhw;
...
public procedure free_reg (reg: rsc);
 var r: registers;
 begin
  r := reg^.loc.offset;
...
public function get_reg (treg: reg_selector; p: bit_range): registers;
 ...
 begin
  ...
  if p <= 36
    then begin
      reg := select_single_reg_for_free (treg);
      clr_reg (reg);
    end
    else begin
...
(* DO MIN MAX OP compiles code for integer or real, min or max operators, given
   ...
   and the precision ("p") of the operands.  A register preference ("treg") may
   be specified. *)

procedure do_min_max_op (treg: reg_selector; exp: expr; cmp: cmp_codes; p: bit_range);
 ...
 begin
  with exp^ do begin
    ...
    (* Load the first operand into a destroyable register.  As we compare this value
       ...
       operand compared against.  In order to keep track of the register as the
       other operands are evaluated (possibly displacing the register), the min/max
       node is tagged as being located in the register, and the node's value is
       reloaded before each compare.  If it was not displaced, then no code is
       generated for the reload. *)
    ...
    for i := 2 to upperbound (operand) do begin
      tag_reg (reg, exp);               (* to save intermediate result *)
      if p = 72 (* compile the subtrees of the operand *)
        then prep_direct (anyreg, operand[i])
        else prepare (anyreg, operand[i]);

      reg := load (treg, exp, p);               (* reload the running value; note that a
                                                   dload is not required as there are no
                                                   remaining uses of the running value. *)
      lock (reg);                               (* make the operand addressible *)
      mem := fetch (operand[i], p);
      free (operand[i]);
      unlock (reg);

      if p = 36                         (* generate a compare and skip *)
    ...
    tag_reg (reg, exp);                 (* remember where result placed *)
  end (* with *) ;
 end;
$PAGE iconstp

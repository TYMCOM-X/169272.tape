 ...
 begin
  case exp^.desc.kind of
    ...
    reals:
     ...
     others:
       if exp^.desc.base = nil
	 then word_size := 1 (* ARBITRARY, but it shouldn't matter *)
...
public function make_def (dtdef_types): def;
 begin
  new (make_def);
...
public procedure gen_source (srcid: source_id; srcindx: int_type);
 var cr: code;
 begin
  new (cr, source);
  cr^.stmtid := srcid;
  ...
  with cr^ do begin
    ...
    if temp_operand.immediate then begin
	 ...
	 (opc in [movei, hlli, hrli, hllzi, hrlzi, hlloi, hrloi, hllei, hrlei,
      ...
      if opc = movei then
	inst.opcode := hrrz (* use of move would lose clear of lh *)
      else
	inst.opcode := opc - imdc;
      temp_operand.indirect := false
    end
 ...
 begin
  while btmp_area.first <> nil do begin
    ...
    with bptr_word^ do begin (* compute the byte pointer offset *)
      bptr.index := sp;
      if bptr.bpoffset <= 377777b
        then bptr.bpoffset := halfword (bptr.bpoffset + bpreloc.relval^.loc.offset)
        else bptr.bpoffset := halfword (bptr.bpoffset + bpreloc.relval^.loc.offset - 1000000b);
      bpreloc.kind := absolute_sc;
    end;

    bscan := bptr_area.first; (* find a matching byte pointer *)
    while (bscan <> nil) andif not
          ( (bscan^.kind = bytepointer) andif (bscan^.bptr.value = bptr_word^.bptr.value) andif
...
begin
  with val_recd do begin
    case kind of
      ...
      scalar_cst:
	store_word (('F', ival), offset, width, fullword);
	...
	gen_component( valp, offset, width );
      ...
      ptr_cst:
	store_word (('X', 0, int_nil), offset, width, fullword);
 ...
 begin
  ...
  with value^ do begin
	...
	if real_prec <= srealprec then
	  store_word (('R', real_val), base_offset, bits_per_unit, realword)
	else begin
	  real_words.rvalue := real_val;
	  store_word (('F', real_words.high), base_offset, bits_per_unit, drealword);
      ...
      string_cst:
        begin
          ...
          words_required := (length (str_val) + chars_per_unit - 1) div chars_per_unit;
      ...
      set_cst:
        begin
          offset := base_offset;
          words_required := (dimension (set_val) + bits_per_unit - 1) div bits_per_unit;
          for i := 1 to words_required do begin
		...
		if flexible and (kind in [arrays, strings]) then begin
		  with value^.elem_vals[field_number].valp^ do begin
		    ...
		    if kind = array_cst then begin
		      for i := 1 to (struc_type^.base_size + bits_per_unit - 1) div bits_per_unit do
			gen_word (area_list, ten_word, fullword);
		      ten_word.value := struc_type^.index_type^.maxval;
		    end
		    else begin
		      for i := 1 to (length (str_val) + chars_per_unit - 1) div chars_per_unit do
			gen_word (area_list, ten_word, fullword);
		      ten_word.value := length (str_val);
		    end;
		  end (* with *);
 ...
 begin
  with value do begin
    case kind of
      ...
      scalar_cst:
	gen_word (area_list, ('F', ival), fullword);
      ...
      ptr_cst:
	gen_word (area_list, ('X', 0, int_nil), fullword);
...
public function gen_cnode (value: val_ptr): rel_syllable  options special(coercions);
 ...
 begin
  if value^.def_addr = nil then begin
    ...
    gen_cnode := reldef (def (value^.def_addr)) (* coerce *)
 end;
 ...
 begin
  with value do begin
    if not ((kind in has_ptr) andif (valp^.def_addr <> nil)) and
      ...
      gen_cval := reldef (def (defp)) (* coerce *)
    else
      gen_cval := reldef (def (valp^.def_addr)) (* coerce *)
  end (* with *)
 end;
...
begin
    ...
    with cr^ do begin
      case kind of
        ...
        instruction..halfwords:
          def_ctr := def_ctr + 1;
        defmark:
          def_value (defname, def_ctr, true);
        others:
          (* no action *)
      end (* case kind *);
      cr := next;
  ...
  if write_it then begin
	...
	case kind of
	  ...
	  source:
	    if (map_opt in cur_block^.semantic_options) and (stmtindx = 1) then
	      map_write (stmtid, ic);
	  others:
	    (* no action *)
	end (* case kind *);

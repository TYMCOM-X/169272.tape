...
(* DO_USE closes and reopens files before doing a USE command.  If
   ...
   open for input, but we were auto_run, then we open the terminal
...
begin
  ...
  rewrite (ttyoutput);
  if (tty <> nilf) orif auto_run then begin
    open (tty);
...
$PAGE do_helper for the HELP command
procedure do_help;

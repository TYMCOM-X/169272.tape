program djw13 options dump(noro);
...
begin
  ...
  if [rec.i_packed_byte..rec.j_packed_byte] <= [k] then
    i := 38;
  if not ([rec.i_packed_byte..rec.j_packed_byte] <= [k]) then
    i := 39;
  if [0..1] <= [k] then
    i := 40;
  if not ([k] >= [0..1]) then
    i := 41;
  if [1..rec.i_packed_byte] <= [j] then
    i := 42;
  if not ([j] >= [1..rec.i_packed_byte]) then
    i := 43;
  if [i..1] <= [j] then
    i := 44;
  if not ([i..1] <= [j]) then
    i := 45;
  if [rec.i_packed_byte..1] <= [rec.j_packed_byte] then
    i := 46;
  if not ([rec.i_packed_byte..1] <= [rec.j_packed_byte]) then
    i := 47;
  bool := [rec.i_packed_byte..rec.j_packed_byte] <= [k];
  bool := not ([rec.i_packed_byte..rec.j_packed_byte] <= [k]);
  bool := [0..1] <= [k];
  bool := not ([k] >= [0..1]);
  bool := [i..1] <= [j];
  bool := not ([i..1] <= [j]);
  bool := [rec.i_packed_byte..1] <= [rec.j_packed_byte];
  bool := not ([rec.i_packed_byte..1] <= [rec.j_packed_byte]);
  ...
  if not ([1..4] <= [k..l]) then
    i := 53;
  if [1..4] <= [i3_b1..j3_b1] then
    i := 54;
  if not ([i3_b1..j3_b1] >= [1..4]) then
    i := 55;
  bool := [i..j] <= [k..l];
  bool := not ([i..j] <= [k..l]);
  bool := [1..j] <= [k..l];
  ...
  bool := [1..4] <= [k..l];
  bool := not ([1..4] <= [k..l]);
  bool := [1..4] <= [i3_b1..j3_b1];
  bool := not ([i3_b1..j3_b1] >= [1..4]);
